...
TypeScript client for the [`retrace-dap`](https://github.com/retracesoftware/retrace-dap)
replay binary's DAP adapter; DAP semantics live in that repo, and Python
replay/control behavior lives in `src/retracesoftware/control_runtime.py`.
...
- Do not duplicate DAP stepping, breakpoint, stack-frame, or variable semantics
  in TypeScript. Those belong in `retrace-dap` and the Python control runtime.
