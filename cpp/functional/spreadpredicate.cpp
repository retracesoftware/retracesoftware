...
namespace {
    ...
    struct SpreadPredicate : public PyObject {
        ...
        static PyObject* call_impl(SpreadPredicate* self, PyObject* const* args, size_t nargsf, PyObject* kwnames) {
            ...
            const Py_ssize_t total = nargs + kwcount;
        ...
        static PyObject* create(PyTypeObject* type, PyObject* args, PyObject* kwds, vectorcallfunc vectorcall) {
            if (kwds && PyDict_GET_SIZE(kwds) != 0) {
                PyErr_Format(PyExc_TypeError, "%S does not accept keyword arguments", type);
                return nullptr;
            }
            if (PyTuple_GET_SIZE(args) != 1) {
                PyErr_Format(PyExc_TypeError, "%S requires exactly one predicate", type);
                return nullptr;
            }

            PyObject* predicate = PyTuple_GET_ITEM(args, 0);
...
PyTypeObject SpreadAnd_Type = {
    ...
    .tp_doc = "spread_and(predicate)\n--\n\n"
              ...
              "Example:\n"
              "    >>> all_positive = spread_and(lambda x: x > 0)\n"
              "    >>> all_positive(1, 2, three=3)\n"
              "    True",
