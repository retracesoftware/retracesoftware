...
and which values are treated as immutable, bound, disabled, or replay-only
stubs. Missing or wrong config here can cause replay divergence without
...
- Directives such as `proxy`, `ext_proxy_result`, `immutable`, `bind`,
  `disable`, `wrap`, `patch_class`, `patch_types`, `type_attributes`,
  `default`, `ignore`, `pathparam`, and `stub_for_replay` affect how
  install/proxy/runtime layers behave.
...
- Wrong `bind` or `wrap` directives that change replay object identity.
...
- If you add or change `bind` or `wrap`, re-check the relevant
  stream/protocol/proxy tests because those directives affect replay identity.
