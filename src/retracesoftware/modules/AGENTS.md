...
and which values are treated as immutable, bound, disabled, or eligible for the
narrow disabled-context materialized replay path. Missing or wrong config here
can cause replay divergence without touching core runtime code.
...
- Directives such as `proxy`, `ext_proxy_result`, `immutable`, `bind`, `disable`,
  `patch_hash`, `wrap`, `patch_class`, `patch_types`, `type_attributes`,
  `default`, `ignore`, `pathparam`, `replay_materialize`, and
  `stub_for_replay` affect how install/proxy/runtime layers behave.
...
- Wrong `replay_materialize`, `bind`, or `wrap` directives that change replay
  object identity or expand materialization beyond disabled-context real-object
  needs.
...
- If you add or change `replay_materialize`, `bind`, or `wrap`, re-check the
  ...
  identity and the disabled-context materialization contract.
